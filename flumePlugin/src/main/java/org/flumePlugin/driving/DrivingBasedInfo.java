 ...
 * @author yuer
 * */
public class DrivingBasedInfo implements AsyncHbaseEventSerializer{
...
//		drivingRowKey.setName(headers.get(NAME));
	...
	public List<PutRequest> getActions() {
		...
		if (isNotNull(drivingRowKey)) {
			putRequests.add(new PutRequest(table, RowKey.createRowKeys(drivingRowKey), columnFamily,"drivingBasedInfo".getBytes(),currentEvent.getBody()));
		}else{
	...
	public List<AtomicIncrementRequest> getIncrements() {
		...
		incrementRequests.add(new AtomicIncrementRequest(table, "drivingbasedinfo".getBytes(), columnFamily, "m".getBytes()) );
